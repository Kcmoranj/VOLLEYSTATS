...
function renderSanciones() {
    ...
    const multasEquipo = (data.multasEquipo || []).filter(m => m.id_equipo === equipo.id);

    // --- Resumen de totales ---
    const totalMultasTarjetas = tarjetas.reduce((acc, s) => acc + (s.multa || 0), 0);
    const totalMultasEquipo = multasEquipo.reduce((acc, m) => acc + (m.monto || 0), 0);
    ...
    document.getElementById('statTotalAPagar').textContent = `$${(totalMultasTarjetas + totalMultasEquipo).toFixed(2)}`;
        ...
        tbodyTarjetas.innerHTML = tarjetas
            ...
            .map(s => {
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="text-right font-black ${s.multa ? 'text-red-600' : 'text-gray-300'}">${s.multa ? '$' + s.multa.toFixed(2) : '—'}</td>
                    <td class="text-xs text-gray-400">${partido ? `vs ${rival || '-'} · ${partido.fecha}` : new Date(s.fecha).toLocaleDateString('es-EC')}</td>
                </tr>`;
            }).join('');
    }

    // --- Tabla de multas al equipo ---
    const tbodyMultas = document.getElementById('tablaMultasEquipo');
    if (multasEquipo.length === 0) {
        tbodyMultas.innerHTML = `<tr><td colspan="3" class="text-center text-gray-400 py-6 font-semibold">Sin multas al equipo. 🎉</td></tr>`;
    } else {
        tbodyMultas.innerHTML = multasEquipo
            .slice()
            .sort((a, b) => new Date(b.fecha) - new Date(a.fecha))
            .map(m => `
                <tr class="hover:bg-gray-50">
                    <td class="text-gray-700 font-semibold">${m.motivo}</td>
                    <td class="text-right font-black text-red-600">$${m.monto.toFixed(2)}</td>
                    <td class="text-xs text-gray-400">${new Date(m.fecha).toLocaleDateString('es-EC')}</td>
                </tr>`).join('');
    }
}