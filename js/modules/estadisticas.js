...
const inicializarDatos = () => {
    ...
    // Prioridad a LocalStorage, si no existe usa el Mock Data general
    data = localData ? JSON.parse(localData) : window.VolleyAppData;

    // FIX (doble fuente de verdad para jugadores): antes acá se sobreescribía
    // data.jugadores con lo guardado en la clave separada 'volley_jugadores'.
    // Como Solicitudes (aprobar/rechazar jugador) solo actualiza 'volleyData',
    // esa segunda copia podía quedar vieja y mostrar estadísticas de jugadores
    // ya rechazados, o no reflejar aprobaciones recientes. 'volleyData' es
    // ahora la única fuente de verdad para jugadores.
};
